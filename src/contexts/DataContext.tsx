...
const DataContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getDataFiltered = React.useCallback(
    (filter: Filter) => {
      // Respect Hidden
      let filtered_data: IProjects = deepcopy(data);
      filtered_data = filtered_data.filter((project) =>
        filter_hidden(filter, project.hidden),
      );
      filtered_data.forEach((project) => {
        project.tickets = project.tickets.filter((ticket) =>
          filter_hidden(filter, ticket.hidden),
        );
      });
      ...
      filtered_data.forEach((project) => {
        project.tickets = project.tickets.filter((ticket) =>
          filter_search(filter, [ticket.code, ticket.name]),
        );
      });
  ...
  const editTicket = (
    ...
    setData((data) => {
      ...
      let [project_index, project] = getProjectIndex(new_data, project_id);
      let [ticket_index] = getTicketIndex(
        new_data,
        project_index,
        prev_ticket_id,
      );
  ...
  const addTicket = (
    project_id: string,
    new_ticket_info: Omit<ITicket, "id">,
  ) => {
    setData((data) => {
      let new_data: IProjects = deepcopy(data);
