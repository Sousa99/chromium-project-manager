...
export const DEFAULT_FEATURE_BRANCH_MODE = '${SEMTYPE}/${CODE}_${NAME_LOWER_NOSPACES}';
...
export const checkValidityMode = (mode_format: string): boolean => {
  const regex = /\${([^}]+)}/g;
  const matches = Array.from(mode_format.matchAll(regex)).map(match => match[1]);

  return matches.every((match) => match in RECOGNIZED_CODES);
}

export const generateModeCode = (mode_format: string, info: TicketInfo): string => {
  const regex = /\${([^}]+)}/g;
  const matches = Array.from(mode_format.matchAll(regex)).map(match => match[1]);
