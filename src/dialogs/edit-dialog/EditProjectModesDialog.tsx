...
export const EditProjectModesDialog = (props: IProps) => {
  const { open, curr_project_modes, onClose } = props;

  const [modeFeatureBranch, setModeFeatureBranch] = React.useState<string>(
    curr_project_modes.feature_branch,
  );
  const [modePullRequest, setModePullRequest] = React.useState<string>(
    curr_project_modes.pull_request,
  );
  const [modeReport, setModeReport] = React.useState<string>(
    curr_project_modes.report,
  );

  const validModeFeatureBranch = React.useMemo(
    () => checkValidityMode(modeFeatureBranch),
    [modeFeatureBranch],
  );
  const validModePullRequest = React.useMemo(
    () => checkValidityMode(modePullRequest),
    [modePullRequest],
  );
  const validModeReport = React.useMemo(
    () => checkValidityMode(modeReport),
    [modeReport],
  );
  ...
  return (
    <Dialog open={open} onClose={_onClose} className="edit-dialog sub-dialog">
      <DialogTitle>{`Advanced Project Configurations`}</DialogTitle>
      <DialogContent>
